...
const FormField: React.FunctionComponent<FormFieldProps> = ({
      ...
      if (domType === 'input') {
        return getFieldDecorator(itemName, {
          rules: [
            {
              required,
              message: requiredTips || `请输入${label}`
            },
            ...rules
          ],
          ...fieldOptions
      ...
      if (domType === 'picker') {
        return getFieldDecorator(itemName, {
          ...
          <Picker title={label} data={pickerData} cols={1}>
            {content || renderPickerDom({ label, itemName, pickerData })}
          </Picker>
        )
      }
      return null
  ...
  return (
    <div className={cx('tui-formfield', className)} onClick={onClick}>
      {extraItem || (
        <div className={cx('control', controlClassName)}>
          <div className={cx('header')}>
            <div className={cx('label', { required })}>{label}</div>
            {type === 'picker'
              ? labelImg ?? <img src={IconRight} alt="IconRight" />
              : labelImg}
          </div>
          <div className={cx('content')}>{renderMainDom(type)}</div>
